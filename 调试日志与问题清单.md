...
#### A) 本地复刻（Windows / PowerShell）

在 `todo-mobile` 目录执行：
```powershell
# 0. 使用 lockfile 还原依赖树（比 npm install 更稳定）
npm ci

# 1. 自证版本（排除“装的不是你以为的版本”）
npm ls react-native --depth=0
npm ls react-native-reanimated --depth=0
npm ls react-native-gesture-handler --depth=0

# 2. 重新生成原生工程（确保依赖变更进入 android/）
if (Test-Path .\android) { Remove-Item -Recurse -Force .\android }
npx expo prebuild --platform android

# 3. 生成离线 bundle（否则 APK 运行会红屏 Unable to load script）
...
#### B) CI 复刻（GitHub Actions / ubuntu-latest）

核心点是“依赖可复现 + 版本可自证 + 先 bundle 再 assemble”。最小脚本形态如下（示意，实际以 workflow 为准）：
```bash
cd todo-mobile
